import { Factor, fetchFeeds, parseFeed, TipsterItem } from "../../core";
import { SingleBar, Presets } from "cli-progress";
import { signals } from "../../services";

const tipsterUrls: TipsterItem[] = [
  {
    factor: "political",
    sources: [
...
export const tipster = async (factor: Factor, limit: number = 5) => {
  ...
  const items: TipsterItem | undefined = tipsterUrls.find(
    (item) => item.factor === factor
  );

  if (items) {
    const feedItems = await fetchFeeds(items.sources, limit, tipsterProgress);
